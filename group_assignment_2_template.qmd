---
title: "Classifying Kangaroo Species Using Skull Morphology Through the Lens of Statistical Machine Learning"
subtitle: "STAT 253"
date: today
author: "Rosie, William, Georgia and Lucia"
format:
  html:
    toc: true
    toc-depth: 3
...
# use penalty to see variable selection
parsiminious_penalty <- lasso_models %>% 
  select_by_one_std_err(metric = "mae", desc(penalty))

final_lasso_model <- lasso_workflow %>% 
  finalize_workflow(parameters = parsiminious_penalty) %>% 
...
log_kangaroo_cv <- logistic_workflow %>%  
  fit_resamples(
    ...
    metrics = metric_set(accuracy, sens, spec, roc_auc)  # AUC averaged from 10 folds
...
# use cross-validation predictions instead of training data predictions
log_predictions_cv <- log_kangaroo_cv %>%
  collect_predictions()
...
# create roc curve using probability predictions
log_predictions_cv %>%
  roc_curve(truth = group_species, .pred_Fuliginosus) %>%
  autoplot() +
  labs(
...
# calculate and display AUC pooled together
log_predictions_cv %>%
  roc_auc(truth = group_species, .pred_Fuliginosus) %>%
  knitr::kable(digits = 3, caption = "Area Under the ROC Curve (AUC)")
```
Our model's accuracy is 83%, meaning that our model's ability of correctly distinguishing between Fuliginosis and Giganteus species is correct 83 percent of the times. Looking at other metrics, the sensitivity of this model is 0.72, meaning that it correctly predicts our reference category Ginganteus as Ginganteus about 72% of the time. In contrast, the specificity is approximately 0.91, indicating that the model correctly predicts Fuliginosus as Fuliginosus about 91% of the time. This difference among sensitivity and specificity makes sense in the context of the data as there are more abundant values for the Fuliginosus species. These results can be calculated manually from the logistic regression confusion matrix to further enhance our results.These three metrics suport that this model's ability to make predictions is strong, specifically for predicting Fuliginosus. 

Another metric to evaluate this model is the Area Under the Curve (AUC), which has a value of 0.914. This indicates that the model can distinguish well between Fuliginosus and Giganteus about 91–92% of the time. While these performance metrics are strong, they might raise concerns about potential overfitting. However, the use of cross-validation and LASSO during model training helps mitigate this risk by testing the model on unseen folds of the data and reducing the number of predictors while preserving accuracy. 

# Contributions

Model exploration was evenly distributed across all four team members. Rosie investigated decision trees and random forests, Lucia explored KNN, and Georgia and William worked together on logistic regression with LASSO variable selection. After running all models, we compared results and collaboratively selected the best model based on accuracy and mapping the two subspecies of the kangaroos together. Additionally, individual contributions included Georgia researching kangaroo taxonomy to support the binary classification, William examining approaches for handling missing data—specifically the use of step_impute_knn, Lucia interpreting and explaining model evaluation metrics, and Rosie troubleshooting issues within the tree and forest algorithms. Finally, we wrote this report collaboratively, reviewing and providing feedback on each section as a group.

# Appendix

# Random Tree and Forest Exploration (Rosie)
<details>
<summary>View Code</summary>
```{r }
#| eval: false
...
# Knn with LASSO exploration (Lucia)
<details>
<summary>View Code</summary>
```{r}
#| eval: false
...
# Decision Tree with LASSO (William)
<details>
<summary>View Code</summary>
```{r}
#| eval: false
